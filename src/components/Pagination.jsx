...
function Products() {
 ...
 const handleSearch = (e) =>{
  ...
  const handleMove = (limit) =>{
    setSkip((preSkip)=>{
     return Math.max(preSkip + limit, 0);
    })  
  }
    ...
    const { data: products } = useQuery({
        ...
        queryFn: async () => {
                let url = `https://dummyjson.com/products/search?limit=${limit}&skip=${skip}&q=${search}`;
               if(category) {
                    ...
                    <div>
                        <div className="relative mt-2 rounded-md flex items-center gap-8 mb-4">
                            <input
                                ...
                                placeholder="Search..."
                            />
                            <select className="border p-2" onChange={(e) => { handleSelectCategory(e) }}>
